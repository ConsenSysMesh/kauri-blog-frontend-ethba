...
module.exports = {
  modify(baseConfig, { target, dev }, webpack) {
    ...
    config.module.rules = config.module.rules.filter(
      rule =>
        !(Array.isArray(rule.use) && rule.use.length > 0 && rule.use[0].options && 'useEslintrc' in rule.use[0].options)
    )
    const babelLoader = config.module.rules.findIndex(rule => rule.use[1].options && rule.use[1].options.babelrc)
